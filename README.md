...
- **Cobalt design system.** Electric indigo (`#6366F1`) on cool charcoal (`#0A0E18`). Geist Sans across the product, Geist Mono for stats and labels — no serif, no italic, no warmth. Tokens codified in three places that stay in sync: [`app/globals.css`](frontend/app/globals.css), [`tailwind.config.ts`](frontend/tailwind.config.ts), [`tokens.ts`](frontend/src/lib/tokens.ts). See [`docs/design-system.md`](docs/design-system.md) for the full spec.
